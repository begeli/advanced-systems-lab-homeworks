...
void kernel_base(double *x, double* re, double* im, int N) {
  int k = 0;
  for (int i = 0; i < N; i+=2) {
      re[k] = x[i];
      im[k] = x[i+1];
...
double perf_test(comp_func f, string desc, int flops)
{
    ...
    static double y[n/2] __attribute__ ((aligned (32)));
    static double h[n/2] __attribute__ ((aligned (32)));
    rands(x, 1, n);
    rands(y, 1, n/2);
    rands(h, 1, n/2);
...
int main(int argc, char **argv)
{
  ...
  int n_base = 1024;
  double *x, *y, *h, *y_old, *y_base, *h_old, *h_base;
  build(&x, 1, n_base);
  build(&y, 1, n_base);
  build(&h, 1, n_base);
  y_base = static_cast<double *>(malloc(n_base * sizeof(double)));
  y_old  = static_cast<double *>(malloc(n_base * sizeof(double)));
  h_base = static_cast<double *>(malloc(n_base * sizeof(double)));
  h_old  = static_cast<double *>(malloc(n_base * sizeof(double)));
  
  for (int k = 0; k < 1; k++) {
    int n = n_base - k;
    memcpy(y_old,  y, n*sizeof(double));
    memcpy(h_old,  h, n*sizeof(double));
    kernel_base(x, h, y, n);
    memcpy(y_base, y, n*sizeof(double));
    memcpy(h_base, h, n*sizeof(double));
  
    for (i = 0; i < numFuncs; i++) {
      memcpy(y, y_old, n*sizeof(double));
      memcpy(h, h_old, n*sizeof(double));
      comp_func f = userFuncs[i];
      f(x, h, y, n);
      double error = nrm_sqr_diff(y, y_base, n) + nrm_sqr_diff(h, h_base, n);
